...
export default {
  ...
  methods: {
    ...
    _initEditor() {
      // 冯子晋改过: 添加调试日志
      console.log("初始化编辑器:", {
        id: this.id || this.editorId || "editor_" + randomString(8),
      ...
      this.editor.addListener("ready", () => {
        ...
        this.editor.addListener("contentChange", () => {
          this._addImageStyles();
        });

        // 初始化时也处理一次图片样式 冯子晋添加的代码
        this._addImageStyles();
    ...
    _loadEditorDependencies() {
      // 冯子晋改过: 添加调试日志
      console.log("开始加载UEditor依赖:", {
        defaultEditorDependencies:
          this.editorDependencies || defaultEditorDependencies,
        UEDITOR_HOME_URL: this.mixedConfig.UEDITOR_HOME_URL,
        window_UE: !!window.UE,
        window_UEDITOR_CONFIG: !!window.UEDITOR_CONFIG,
      });
    ...
    // 添加处理图片样式的方法 冯子晋添加的代码
    _addImageStyles() {
      try {
        const editorDocument = this.editor.document;
        const images = editorDocument.getElementsByTagName("img");
        for (let i = 0; i < images.length; i++) {
          images[i].style.maxWidth = "100%";
          images[i].style.height = "auto";
        }
      } catch (error) {
        console.error("添加图片样式失败:", error);
      }
    },
  },
  deactivated() {
