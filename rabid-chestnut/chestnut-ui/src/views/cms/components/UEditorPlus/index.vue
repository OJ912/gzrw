<template>
  <div class="ueditor">
    <!-- editorDependencies变成了editor-dependencies 冯子晋修改代码 -->
    <vue-ueditor-wrap
      v-model="contentHtml"
      :editor-id="editorId"
      :config="editorConfig"
...
// 添加调试代码
console.log(
  "%c UEditor Path Debug Info ",
  "background:#3498db; color:#fff; padding: 5px; border-radius: 3px; font-weight: bold;"
);
...
// 添加资源加载调试函数
function debugResourceLoading(resourceUrl) {
  const startTime = new Date().getTime();
  return fetch(resourceUrl, { method: "HEAD" })
    .then((response) => {
      const endTime = new Date().getTime();
      console.log(
        `%c Resource Check: ${resourceUrl}`,
    ...
    .catch((error) => {
      console.error(`Failed to check resource: ${resourceUrl}`, error);
      return false;
    });
}

// 冯子晋修改了这一行 const UE_HOME = "/UEditorPlus/";
const UE_HOME = process.env.VUE_APP_PATH + "UEditorPlus/";
console.log(
  "%c Final UE_HOME ",
  "background:#e74c3c; color:#fff; padding: 5px; border-radius: 3px; font-weight: bold;",
  UE_HOME
);
...
export default {
  ...
  methods: {
    ...
    handleBeforeInit(editorId) {
      // 冯子晋改过: 添加调试日志
      console.log("UEditor 初始化前状态:", {
        editorId,
        window_UE: !!window.UE,
        window_UEDITOR_CONFIG: !!window.UEDITOR_CONFIG,
