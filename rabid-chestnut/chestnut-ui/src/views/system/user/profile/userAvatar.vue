    ...
    <el-dialog :title="title" :visible.sync="open" width="800px" append-to-body @opened="modalOpened"  @close="closeDialog">
      ...
      <el-row>
        ...
        <el-col :lg="{span: 2, offset: 6}" :sm="2" :xs="2">
          <el-button type="primary" size="small" @click="uploadImg()">{{ $t('Common.Save') }}</el-button>
        </el-col>
      </el-row>
    </el-dialog>
  </div>
...
export default {
  ...
  data() {
    return {
      ...
      visible: false,
      // 弹出层标题
      title: this.$t('System.User.EditAvatar'),
      options: {
  ...
  methods: {
    ...
    beforeUpload(file) {
      if (file.type.indexOf("image/") == -1) {
        this.$modal.msgError(this.$t('Common.InvalidFileSuffix', [ '.jpg, .png' ]));
      } else {
